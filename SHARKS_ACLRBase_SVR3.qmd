...
```{r}
# Create a memoised version of powerSim with progress tracking
create_memoised_powersim <- function() {
  
  # Create progress bar function
  progress_callback <- function(i, total, start_time) {
    if (i == 1) {
      pb <<- progress_bar$new(
        format = "Power Simulation [:bar] :percent in :elapsed, eta: :eta",
        total = total,
        clear = FALSE,
        width = 60
      )
  ...
  # Wrapper function for powerSim that includes progress tracking
  powersim_with_progress <- function(model, test, nsim = 100, seed = NULL, ...) {
    
    # Set seed for reproducibility
    if (!is.null(seed)) set.seed(seed)
    
    cat("Starting power simulation with", nsim, "iterations...\n")
    start_time <- Sys.time()
    
    # Run powerSim with progress callback
    result <- powerSim(
      model, 
      test = test, 
      nsim = nsim,
  ...
  memoised_powersim <- memoise(powersim_with_progress)
  
  return(memoised_powersim)
}

# Initialize the memoised function
memo_powersim <- create_memoised_powersim()

# Example usage for your model
run_power_analysis <- function(model, effect_name, nsim = 100, seed = 123) {
  
  # Define the test based on effect name
  test <- switch(effect_name,
    "Age_c" = fixed("Age_c"),
    "Age_c_squared" = fixed("I(Age_c^2)"),
    "Sex" = fixed("Sex_fMale"),
    "BMI" = fixed("BMI_c"),
    "MeniscusCartilage" = fixed("MeniscusCartilage_fYes"),
    "comorbidity" = fixed("comorbidity_count"),
    "ses" = fixed("ses_score_c"),
    "education" = fixed("education_years_c"),
    "study_type_Prospective" = fixed("study_typeProspective_Cohort"),
    "study_type_RCT" = fixed("study_typeRCT"),
    "setting_Community" = fixed("setting_typeCommunity_Clinic"),
    "setting_Private" = fixed("setting_typePrivate_Practice"),
    "setting_Sports" = fixed("setting_typeSports_Medicine_Clinic"),
    "capture_Electronic" = fixed("capture_methodElectronic"),
    stop("Unknown effect name")
  )
  
  # Run memoised power simulation
  result <- memo_powersim(
    model = model,
    test = test,
    nsim = nsim,
    seed = seed
...
```{r}
# Function to run power analysis for multiple effects
run_multiple_power_analyses <- function(model, effects, nsim = 100, seed = 123) {
  
  results <- list()
  
  for (effect in effects) {
    cat("\n", paste(rep("=", 50), collapse = ""), "\n")
    cat("Running power analysis for:", effect, "\n")
    cat(paste(rep("=", 50), collapse = ""), "\n")
    
    results[[effect]] <- run_power_analysis(model, effect, nsim, seed)
    
    # Print summary
    cat("Power for", effect, ":", 
        round(results[[effect]]$power, 3), 
        "(", results[[effect]]$successes, "/", results[[effect]]$trials, ")\n")
  }
...
# Load your data and fit simplified model
simplified_model <- lmer(
  IKDC ~ Age_c + I(Age_c^2) + Sex_f + BMI_c + MeniscusCartilage_f + 
         (1|idStudy_f),
  data = dtAnalysis2
)

# Run power analysis for key effects
key_effects <- c("Age_c", "Sex", "BMI", "MeniscusCartilage")
power_results <- run_multiple_power_analyses(simplified_model, key_effects, nsim = 100)

```

```{r}
# Debug function to test a single effect first
test_single_power_analysis <- function(model, effect_name, nsim = 10) {
  cat("Testing power analysis for:", effect_name, "\n")
  
  tryCatch({
    result <- run_power_analysis(model, effect_name, nsim)
...
```{r}
test_result <- test_single_power_analysis(ikdc_model, "Age_c", nsim = 10)
```
