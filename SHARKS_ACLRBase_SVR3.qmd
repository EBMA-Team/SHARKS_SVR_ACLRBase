...
Redo the simulation to reduce the sample size and complexity initially.

We'll recreate the model using simr and insert effect sizes based on the model just presented.

```{r}

# we'll grab the dtAnalysis structure and slice out some covariates
# 
...
Construct the covariate data.frame by slicing out the requisite columns from dtAnalysis

```{r}

dtCovar <- dtAnalysis2 |> dplyr::select(
  idPatient,
  idStudy_f,
  idRegion_f,
  Age_c,
...
Construct model parameters from previous simulation
...
Create lmer model from scratch

```{r}

SimModel <- makeLmer(IKDC ~ 
    # Fixed effects - patient case-mix variables
    Age_c +
    BMI_c +
    MeniscusCartilage_f + 
...
```{r}
sim_Sex <- powerSim(SimModel, nsim=10, test = fcompare(IKDC~Sex_f))

sim_Sex
```

```{r}
# Check the structure of your factor
str(dtCovar$idStudy_f)
levels(dtCovar$idStudy_f)
length(levels(dtCovar$idStudy_f))

# Check how many studies you currently have vs. trying to extend to
current_n_studies <- length(levels(dtCovar$idStudy_f))
print(paste("Current studies:", current_n_studies, "Extending to:", 20))
```

```{r}
SimModel_ext_study <- extend(
  SimModel, 
  along = "idStudy_f", 
  n = 60
  )
...
p_curve_treat1 <- powerCurve(
  SimModel_ext_study,
  test=fcompare(IKDC~Sex_f),
  along="idStudy_f",
  ncores = n_cores,  # Use parallel processing
...
```{r}
Exmodel <- makeLmer(y ~ treat*time + (1|class/id), fixef=fixed, VarCorr=rand, sigma=res, data=covars)
Exmodel
```

```{r}
Exsim_treat <- powerSim(Exmodel, nsim=100, test = fcompare(y~time))
Exsim_treat
```

```{r}
sim_time <- powerSim(Exmodel, nsim=100, test = fcompare(y~treat))
sim_time
```

```{r}
Exmodel_ext_class <- extend(Exmodel, along = "class", n = 20)
Exmodel_ext_class

nrow(getData(Exmodel_ext_class))
nrow(getData(Exmodel))
```

```{r}
p_curve_treat <- powerCurve(Exmodel_ext_class, test=fcompare(y~time), along="class")
plot(p_curve_treat)
```
